...
var wasPlayerInvitedWithTheGameClosed: bool = false
var lobbyId: int = 0
...
func OnLobbyChatUpdate(_lobbyId: int, _playerId: int, _makingChangeId: int, _chatState: int):
	fetchLobbyMembers()
...
func checkIfPlayerWasInvitedWithTheGameClosed():
	...
	if (args.size() > 0 && args[0] == '+connect_lobby' && int(args[1]) > 0):
		wasPlayerInvitedWithTheGameClosed = true
